...
def _lbl(key: str) -> str:
    ...
    _labels = {
        "product_plural":   "פוליסות חיסכון" if is_policy else "קרנות פנסיה" if is_pension else "קרנות",
        "product_singular": "פוליסת חיסכון"  if is_policy else "קרן פנסיה"   if is_pension else "קרן",
        "fund_count_lbl":   "פוליסות"         if is_policy else "קרנות"        if is_pension else "קופות",
        "manager_lbl":      "מנהל ההשקעות"    if is_policy else "מנהל"         if is_pension else "מנהל",
...
""", unsafe_allow_html=True)
    c1, c2, c3, _ = st.columns([1.4, 1.6, 1.4, 4])
    with c1:
